...
class MultiExperimentHandler:
    ...
    def __init__(self, output_dir=None):
        ...
        """
        self.output_dir = (
            output_dir or f"experiments_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        if not os.path.exists(self.output_dir):
            os.makedirs(self.output_dir)

        self.results = {"binary": {}, "multi": {}, "regression": {}}
    ...
    def run_binary_experiments(
        ...
        n_jobs=-1,
        random_state=42,
        optimize_negative_recall=False,
    ):
        """
        ...
        for algorithm in tqdm(algorithms, desc="Binary Classification", unit="models"):
            ...
            X_train, X_test, y_train, y_test = model.train_test_split(
                test_size=test_size, random_state=random_state, stratify=model.y
            )
            ...
            grid_search = model.perform_grid_search(
                X_train, y_train, algorithm, cv=cv, n_jobs=n_jobs
            )
            ...
            if optimize_negative_recall:
                ...
                if (
                    hasattr(model, "grid_search_results")
                    and "best_model" in model.grid_search_results
                ):
                    ...
                    if hasattr(best_model, "predict_proba"):
                        ...
                        optimal_threshold, achieved_recall, threshold_results = (
                            model.find_threshold_for_negative_class_recall(
                        ...
                        for thresh in thresholds:
                            ...
                            f1 = (
                                2 * (precision * pos_recall) / (precision + pos_recall)
                                if (precision + pos_recall) > 0
                                else 0
                            )
                        ...
                        evaluation_results = model.evaluate_model(
                            X_test, y_test, prediction_threshold=optimal_threshold
                        )
                    else:
                        # For models without probabilistic outputs, use default evaluation
                        evaluation_results = model.evaluate_model(X_test, y_test)
                        print(
                            f"Warning: Model {algorithm} does not support probability outputs. Using default threshold."
                        )
                else:
                    # Fallback if model structure is different
                    evaluation_results = model.evaluate_model(X_test, y_test)
                    print(
                        f"Warning: Model structure for {algorithm} is not as expected. Using default evaluation."
                    )
            else:
            ...
            results[algorithm] = {
                ...
                "minority_class_precision": evaluation_results[
    ...
    def _find_optimal_threshold_for_negative_recall(
        self, y_true, y_prob, num_thresholds=100
    ):
        """
        ...
        print(
            f"Optimal threshold for negative class recall: {best_threshold:.4f} (Negative recall: {best_recall:.4f})"
        )
    ...
    def _plot_roc_curve_with_threshold(
        self, y_true, y_prob, threshold, plots_dir, algorithm
    ):
        """
        ...
        # Save the plot
        plt.savefig(
            os.path.join(plots_dir, "roc_curve_threshold.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()

    def _plot_precision_recall_curve_with_threshold(
        self, y_true, y_prob, threshold, plots_dir, algorithm
    ):
        """
        ...
        plt.title(
            f"Precision-Recall Curve with Optimal Threshold ({algorithm})", fontsize=14
        )
        plt.legend(loc="best")
        plt.grid(alpha=0.3)
        ...
        # Save the plot
        plt.savefig(
            os.path.join(plots_dir, "precision_recall_curve_threshold.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _plot_confusion_matrix(self, y_true, y_pred, plots_dir, algorithm):
        ...
        # Save the plot
        plt.savefig(
            os.path.join(plots_dir, "confusion_matrix_threshold.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _plot_threshold_metrics(self, plots_dir, algorithm):
        ...
        plt.plot(
            df["threshold"],
            df["neg_recall"],
            label="Negative Class Recall",
            linewidth=2,
        )
        plt.plot(
            df["threshold"],
            df["pos_recall"],
            label="Positive Class Recall",
            linewidth=2,
        )
    ...
    def plot_negative_recall_vs_threshold(self, save=True):
        ...
        if not hasattr(self, "threshold_curves") or not self.threshold_curves:
            print(
                "No threshold curves available. Run with optimize_negative_recall=True first."
            )
            return None
        ...
        for algorithm, curve_data in self.threshold_curves.items():
            plt.plot(
                curve_data["threshold"],
                curve_data["neg_recall"],
                linewidth=2,
                label=f"{algorithm}",
            )
    ...
    def run_regression_experiments(
        self, algorithms, target="G3", test_size=0.33, cv=5, n_jobs=-1, random_state=42
    ):
        """
    ...
    def plot_binary_comparison(self, metrics=None, save=True):
        ...
        has_threshold = any(
            "probability_threshold" in results[alg] for alg in algorithms
        )
        title_suffix = (
            " (with Optimized Negative Recall Threshold)" if has_threshold else ""
        )
        plt.title(
            f"Binary Classification Performance Comparison{title_suffix}", fontsize=16
        )
    ...
    def plot_regression_comparison(self, metrics=None, save=True):
        ...
        if not df.empty:
            ...
            axes[0].set_title(
                "Regression Accuracy Metrics (Higher is Better)", fontsize=16
            )
    ...
    def plot_binary_minority_class_metrics(self, save=True):
        ...
        plt.title(
            f"Performance Metrics for Failing Students (Minority Class){title_suffix}",
            fontsize=16,
        )
        ...
        if save:
            filepath = os.path.join(
                self.output_dir, "binary_minority_class_metrics.png"
            )
            plt.savefig(filepath, dpi=300, bbox_inches="tight")
    ...
    def plot_threshold_comparison(self, save=True):
        ...
        results = self.results["binary"]
        has_threshold = any(
            "probability_threshold" in results[alg] for alg in results.keys()
        )

        if not has_threshold:
            print(
                "No threshold data available. Run with optimize_negative_recall=True first."
            )
            return None
        ...
        ax2 = ax1.twinx()
        ax2.plot(
            algorithms, neg_recalls, "ro-", linewidth=2, label="Negative Class Recall"
        )
        ...
        plt.title(
            "Optimal Probability Thresholds for Negative Class Recall", fontsize=16
        )
        plt.xlabel("Algorithm", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run multiple student performance prediction experiments"
    )
    ...
    parser.add_argument(
        "--test_size", type=float, default=0.33, help="Test set size (default: 0.33)"
    )
    ...
    if args.algorithms[0] == "all":
        ...
        binary_algorithms = [
            a for a in args.algorithms if a in classification_algorithms
        ]
        multi_algorithms = [
            a for a in args.algorithms if a in classification_algorithms
        ]
        regression_algorithms = [
            a for a in args.algorithms if a in regression_algorithms
        ]
