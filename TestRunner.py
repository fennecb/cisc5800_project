...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python TestRunner.py --model_type binary --algorithm random_forest --save_results",
    )
    ...
    parser.add_argument(
        "--save_results", action="store_true", help="Save experiment results to disk"
    )
    ...
    if args.model_type == "regression" and args.algorithm not in regression_algorithms:
        ...
        print(
            f"Available algorithms for regression: {', '.join(regression_algorithms)}"
        )
        sys.exit(1)
    elif (
        args.model_type in ["binary", "multi"]
        and args.algorithm not in classification_algorithms
    ):
        print(f"Error: {args.algorithm} is not available for classification.")
        print(
            f"Available algorithms for classification: {', '.join(classification_algorithms)}"
        )
        sys.exit(1)
    ...
    if args.save_results:
        if args.ensemble_voting:
            ...
            experiment_dir, plots_dir = setup_experiment_dir(
                args.model_type, args.algorithm, args.imbalance_method
            )
    ...
    if args.model_type == "binary":
        ...
        print(
            f"Multi-class classification with {args.n_bins} bins using {args.strategy} strategy"
        )
    else:  # regression
    ...
    X_train, X_test, y_train, y_test = model.train_test_split(
        test_size=args.test_size, random_state=args.random_state, stratify=stratify
    )
    ...
    if args.model_type == "binary" and args.imbalance_method == "ensemble":
        ...
        grid_search = model.perform_grid_search(
            X_train, y_train, args.algorithm, cv=args.cv, n_jobs=args.n_jobs
        )
    ...
    if args.ensemble_voting == "soft":
        ...
        results = model.evaluate_model(
            X_test, y_test, prediction_threshold=args.ensemble_threshold
        )
        ...
        results = model.evaluate_model(
            X_test, y_test, prediction_threshold=args.prediction_threshold
        )
    else:
    ...
    if args.model_type == "binary":
        ...
        if args.target_precision is not None:
            ...
            try:
                threshold, actual_precision, threshold_results = (
                    model.find_threshold_for_precision(
                ...
                print(
                    f"F1 score at this precision: {threshold_results['f1_score']:.3f}"
                )
            ...
            threshold, actual_accuracy, threshold_results = (
                model.find_threshold_for_negative_class_accuracy(
            ...
            threshold, actual_recall, threshold_results = (
    ...
    if args.imbalance_method == "ensemble":
        ...
        for model_name, scores in (
            model.grid_search_results["best_scores"]
            .get("individual_models", {})
            .items()
        ):
            print(
                f"  {model_name}: recall_failing={scores.get('recall_failing', 'N/A'):.4f}, "
    ...
    if args.model_type == "binary":
        ...
        print(
            f"Threshold for max precision: {insights['threshold_for_max_precision']:.3f}"
        )
        if insights["high_precision_threshold_90"]:
            print(
                f"Threshold for 90%%+ precision: {insights['high_precision_threshold_90']:.3f}"
            )
        ...
        if args.target_precision is not None and hasattr(
            model, "custom_threshold_results"
        ):
            print("\nAnalyzing students predicted to fail with custom threshold...")
            threshold = model.custom_threshold_results["threshold"]
            failing_students, summary = model.analyze_failing_students(
                X_test, threshold=threshold
            )
            ...
            if "important_features" in summary:
                ...
                for feature, stats in summary["important_features"][:5]:
                    print(
                        f"  {feature}: importance={stats['importance']:.3f}, mean={stats['mean_value']:.3f}"
                    )
