...
class MultiClassification(BaseStudentPerformance):
    ...
    def perform_grid_search(self, X_train, y_train, classifier_name, param_grid=None, cv=5, n_jobs=-1):
        """Perform grid search for multi-class classification"""
        # Get the parameter grid
        if param_grid is None:
            param_grid_enum = getattr(ParamGrids, f"{classifier_name.upper()}_MULTICLASS", None)
                ...
                if true_class != pred_class:
                    ...
                    if count > 0:
                        misclassified_analysis.append({
                            ...
                            'count': count,
                            'percentage': count / (y_test == true_class).sum() * 100
                        })
        ...
        for idx, class_label in enumerate(classes):
            if idx < len(axes):
                ...
                if class_mask.sum() > 0:
                    ...
                    axes[idx].hist(class_confidences, bins=20, alpha=0.7, edgecolor='black')
                    axes[idx].axvline(x=1/len(classes), color='red', linestyle='--', 
                                    label=f'Random ({1/len(classes):.3f})')
                    axes[idx].set_title(f'Prediction Confidence for Class {class_label}')
