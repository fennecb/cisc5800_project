...
from sklearn.preprocessing import StandardScaler, OneHotEncoder, OrdinalEncoder, FunctionTransformer, label_binarize
...
class Regression(BaseStudentPerformance):
    ...
    def perform_grid_search(self, X_train, y_train, regressor_name, param_grid=None, cv=5, n_jobs=-1):
        """Perform grid search for regression"""
        # Get the parameter grid
        if param_grid is None:
            param_grid_enum = getattr(ParamGrids, f"{regressor_name.upper()}_REGRESSION", None)
    ...
    def plot_results(self, figsize=(16, 12)):
        ...
        axes[1, 0].plot(x, stats.norm.pdf(x, mu, sigma), 'r-', lw=2, label=f'Normal(μ={mu:.2f}, σ={sigma:.2f})')
        ...
        axes[1, 0].text(0.05, 0.95, f'Normality test p-value: {p_value_normality:.4f}', 
        ...
        axes[2, 0].text(0.05, 0.95, metrics_text, fontsize=11, verticalalignment='top',
    ...
    def plot_residual_analysis(self, figsize=(15, 10)):
        ...
        axes[1, 1].text(0.95, 0.95, f'Outliers: {len(outliers)}', transform=axes[1, 1].transAxes,
    ...
    def plot_prediction_error_distribution(self, figsize=(12, 8)):
        ...
        if non_zero_mask.sum() > 0:
            percentage_errors = np.where(non_zero_mask, 
        ...
        axes[0, 0].axvline(x=np.mean(abs_errors), color='r', linestyle='--', 
                        label=f'Mean: {np.mean(abs_errors):.2f}')
        axes[0, 0].axvline(x=np.median(abs_errors), color='g', linestyle='--', 
        ...
        if len(percentage_errors_clean) > 0:
            ...
            IQR = Q3 - Q1
            outlier_mask = (percentage_errors_clean >= Q1 - 1.5*IQR) & (percentage_errors_clean <= Q3 + 1.5*IQR)
            percentage_errors_filtered = percentage_errors_clean[outlier_mask]
            
            axes[0, 1].hist(percentage_errors_filtered, bins=30, alpha=0.7, edgecolor='black')
            axes[0, 1].axvline(x=0, color='r', linestyle='-', lw=2)
            if len(percentage_errors_filtered) > 0:
                axes[0, 1].axvline(x=np.mean(percentage_errors_filtered), color='g', linestyle='--', 
                                label=f'Mean: {np.mean(percentage_errors_filtered):.2f}%')
            axes[0, 1].set_xlabel('Percentage Error (%)')
            axes[0, 1].set_ylabel('Count')
            axes[0, 1].set_title(f'Distribution of Percentage Errors\n(Excluding {np.sum(~outlier_mask)} outliers)')
            axes[0, 1].legend()
            
            # Add note about zero values if any
            n_zeros = np.sum(y_test == 0)
            if n_zeros > 0:
                axes[0, 1].text(0.95, 0.95, f'Note: {n_zeros} samples with actual=0 excluded', 
                            transform=axes[0, 1].transAxes, ha='right', va='top',
                            bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))
        else:
            axes[0, 1].text(0.5, 0.5, 'No valid percentage errors\n(all actual values are 0)', 
    ...
    def analyze_prediction_intervals(self, X_test, confidence=0.95):
        ...
        ax.fill_between(x_plot, y_pred_lower[sort_idx], y_pred_upper[sort_idx], 
                    alpha=0.3, color='blue', label=f'{int(confidence*100)}% Prediction Interval')
        ...
        ax.scatter(x_plot, y_test_array[sort_idx], color='red', alpha=0.6, s=10, label='Actual')
