...
class BinaryClassification(BaseStudentPerformance):
    ...
    def create_pipeline(self, classifier_name):
        ...
        if (
            hasattr(classifier, "class_weight")
            and self.imbalance_method == "class_weight"
        ):
        ...
        if self.imbalance_method == "ensemble":
            ...
            for model_name in self.ensemble_models:
                ...
                if param_grid is None:
                    model_param_grid_enum = getattr(
                        ParamGrids, f"{model_name.upper()}_BINARY", None
                    )
                    ...
                    if model_param_grid_enum is None:
                        model_param_grid_enum = getattr(
                            ParamGrids, model_name.upper(), None
                        )
                    if model_param_grid_enum is None:
                        print(
                            f"No parameter grid found for {model_name}, using default parameters"
                        )
                        model_param_grid = {}
                    else:
                        ...
                        if "classifier__class_weight" in model_param_grid:
                            ...
                            model_param_grid["classifier__class_weight"] = (
                                class_weight_options
                            )
                ...
                failing_recall_scorer = make_scorer(
                ...
                refit_metric = (
                    optimize_for if optimize_for in scoring else "recall_failing"
                )
                ...
                best_individual_scores[model_name] = {
                    ...
                    "recall_failing": grid_search.cv_results_[
                        "mean_test_recall_failing"
                    ][grid_search.best_index_],
                    "precision_failing": grid_search.cv_results_[
                        "mean_test_precision_failing"
                    ][grid_search.best_index_],
                    "f1_failing": grid_search.cv_results_["mean_test_f1_failing"][
                        grid_search.best_index_
                    ],
                }

                print(
                    f"{model_name} best score ({refit_metric}): {best_individual_scores[model_name][refit_metric]:.3f}"
                )
            ...
            self.grid_search_results = {
                ...
                "best_params": {
                    model: info["params"]
                    for model, info in best_individual_scores.items()
                },
                "best_scores": {"individual_models": best_individual_scores},
            ...
            print(
                f"Prediction threshold: {self.ensemble_threshold}"
            ...
            if param_grid is None:
                ...
                if (
                    self.imbalance_method == "class_weight"
                    and "classifier__class_weight" in param_grid
                ):
                    # Define various class weight ratios to try
                    class_weight_options = [
                        {0: 5, 1: 1},
            ...
            print(
                f"Recall for failing students: {self.grid_search_results['best_scores']['recall_failing']:.3f}"
            )

            return grid_search

    def evaluate_model(
        self, X_test, y_test, grid_search=None, prediction_threshold=0.5
    ):
        ...
        metrics["minority_class_precision"] = precision_score(
            y_test, y_pred, pos_label=minority_class
        )
        metrics["minority_class_recall"] = recall_score(
            y_test, y_pred, pos_label=minority_class
        )
        metrics["minority_class_f1"] = f1_score(
            y_test, y_pred, pos_label=minority_class
        )
    ...
    def plot_results(self, X_test=None):
        ...
        if self.imbalance_method == "ensemble" and hasattr(self, "fitted_pipelines"):
            ...
            for model_name, pipeline in self.fitted_pipelines.items():
                ...
                minority_precision = precision_score(
                    y_test, model_preds, pos_label=0, zero_division=0
                )
            ...
            minority_precisions.append(
                self.evaluation_results["minority_class_precision"]
            )
            ...
            plt.title(
                "Minority Class (Failing Students) Performance Comparison", fontsize=14
            )
        ...
        precision, recall, _ = precision_recall_curve(
            y_test, y_pred_proba[:, 0], pos_label=0
        )
        plt.plot(recall, precision, marker=".", linewidth=2)
        plt.xlabel("Recall", fontsize=12)
        plt.ylabel("Precision", fontsize=12)
        ...
        plt.title(
            f"ROC Curve (AUC: {self.evaluation_results['roc_auc']:.3f})", fontsize=14
        )
        ...
        if self.imbalance_method != "ensemble" and hasattr(
            self.grid_search_results["best_model"].named_steps["classifier"],
    ...
    def plot_threshold_analysis(self, figsize=(10, 6)):
        ...
        plt.plot(
            thresholds, precisions, label="Precision (Failing Students)", marker="o"
        )
    ...
    def get_misclassified_samples(self, X_test, include_proba=True):
        ...
        if include_proba:
            misclassified_df = misclassified_df.sort_values(
                "prob_pass", ascending=False
            )

        return misclassified_df

    def find_threshold_for_precision(
        self, target_precision, X_test, y_test, min_samples_per_class=10
    ):
        """
        ...
        for threshold in thresholds:
            ...
            if (
                n_predicted_fail < min_samples_per_class
                or n_predicted_pass < min_samples_per_class
            ):
                continue

            # Calculate metrics with zero_division parameter to avoid warnings
            precision = precision_score(
                y_test_stored, y_pred_threshold, pos_label=0, zero_division=0
            )
            recall = recall_score(
                y_test_stored, y_pred_threshold, pos_label=0, zero_division=0
            )
        ...
        if not precisions:
            raise ValueError(
                "No valid thresholds found. Try adjusting min_samples_per_class."
            )
    ...
    def plot_precision_threshold_trade_off(self, figsize=(12, 8)):
        ...
        ax1.plot(
            ...
            precisions,
            "b-",
            label="Precision (Failing Students)",
            linewidth=2,
        )
        ax1.plot(
            valid_thresholds,
        ...
        ax1.plot(
            ...
            f1_scores,
            "r-",
            label="F1 Score (Failing Students)",
            linewidth=2,
        )
        ...
        ax1.set_title(
            "Performance Metrics vs Threshold for Identifying Failing Students\n(Higher threshold = More conservative in predicting pass)"
        )
        ax1.legend()
        ...
        ax2.plot(
            ...
            n_predicted_pass,
            "g-",
            label="Predicted as Passing",
            linewidth=2,
        )
        ax2.axhline(
            y=np.sum(y_test == 0), color="red", linestyle=":", label="Actual Failing"
        )
        ax2.axhline(
            y=np.sum(y_test == 1), color="green", linestyle=":", label="Actual Passing"
        )
        ...
        try:
            ...
            if high_precision_thresholds:
                high_precision_threshold = valid_thresholds[
                    precisions.index(max(high_precision_thresholds))
                ]
                recall_at_90_precision = recalls[
                    precisions.index(max(high_precision_thresholds))
                ]
            else:
        ...
        insights = {
            ...
            "threshold_for_max_precision": valid_thresholds[np.argmax(precisions)]
            if precisions
            else None,
    ...
    def analyze_failing_students(self, X_test, threshold=None, target_precision=0.9):
        ...
        if threshold is None:
            threshold, actual_precision, _ = self.find_threshold_for_precision(
                target_precision, X_test, None
            )
            print(
                f"Using threshold {threshold:.3f} for {actual_precision:.1%} precision"
            )
        ...
        failing_students["fail_probability"] = (
            1 - failing_probabilities
        )  # Convert to probability of failing
        failing_students["predicted_class"] = 0

        # Sort by confidence (probability of failing)
        failing_students = failing_students.sort_values(
            "fail_probability", ascending=False
        )
        ...
        if self.imbalance_method != "ensemble" and hasattr(
            ...
            summary["important_features"] = sorted(
                feature_stats.items(), key=lambda x: x[1]["importance"], reverse=True
            )[:10]
        elif self.imbalance_method == "ensemble":
            # For ensemble models, add a note about feature importance
            summary["ensemble_note"] = (
                "Feature importance not available for ensemble models"
            )

        return failing_students, summary

    def find_threshold_for_negative_class_accuracy(
        self, target_accuracy, X_test, y_test=None, min_samples=10
    ):
        """
        ...
        if not negative_accuracies:
            raise ValueError(
                "No valid thresholds found. Try adjusting min_samples parameter."
            )
    ...
    def find_threshold_for_negative_class_recall(
        self, target_recall, X_test, y_test=None, min_samples=10
    ):
        """
