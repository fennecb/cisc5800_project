def plot_model_comparison(models_results, output_path='model_comparison_chart.png'):
    ...
    metrics = ['accuracy', 'balanced_accuracy', 'f1_score', 'precision', 'recall', 'roc_auc']
    metrics_display = ['Accuracy', 'Balanced Accuracy', 'F1 Score', 'Precision', 'Recall', 'ROC AUC']
...
def plot_precision_recall_curves(models_proba, y_test, output_path='precision_recall_curves.png'):
    """
...
def plot_feature_importance(model, feature_names, top_n=10, output_path='feature_importance.png'):
    """
...
def plot_probability_distribution(y_proba, y_test, threshold=0.5, optimal_threshold=None, 
    ...
    # Plot histograms
    plt.hist(prob_failing, bins=20, alpha=0.6, color='red', 
             label=f'Failing Students (n={len(prob_failing)})')
    plt.hist(prob_passing, bins=20, alpha=0.6, color='green', 
...
def plot_threshold_analysis(y_proba, y_test, thresholds=None, 
    ...
    for threshold in thresholds:
        ...
        y_pred = (y_proba < threshold).astype(int)  # If y_proba < threshold, predict class 0 (fail)
    ...
    # Plot
    plt.figure(figsize=(12, 8))
    plt.plot(thresholds, precisions, 'b-', label='Precision (Failing Students)', linewidth=2)
    plt.plot(thresholds, recalls, 'g-', label='Recall (Failing Students)', linewidth=2)
    plt.plot(thresholds, f1_scores, 'r-', label='F1 Score (Failing Students)', linewidth=2)
