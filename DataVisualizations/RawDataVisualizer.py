...
class RawDataVisualizer:
    ...
    def feature_distribution_visualization(self):
        ...
        for feature in cat_features:
            if len(feature_labels[feature]) == 2:
                ...
                # Add text
                plt.text(101, y_pos, f'Imbalance: {imbalance_ratio:.2f}:1',
        ...
        plt.tight_layout()
        plt.savefig('DataVisualizations/feature_distribution_horizontal.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        for i, feature in enumerate(cat_features):
            ...
            ax.set_xticklabels(counts.index, rotation=45 if len(counts) > 2 else 0, ha='right')
            
            # Add count and percentage on bars
            for j, (bar, count, pct) in enumerate(zip(bars, counts, percentages)):
                height = bar.get_height()
                ax.text(j, height + 1, f'{count}\n({pct:.1f}%)', 
            ...
            if len(counts) == 2:
                imbalance_ratio = counts.max() / counts.min()
                ax.text(0.5, 0.02, f'Imbalance ratio: {imbalance_ratio:.2f}:1',
        ...
        plt.tight_layout()
        plt.savefig('DataVisualizations/feature_distribution_individual.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def class_distribution_barcharts(self):
        ...
        selected_features = ['school', 'sex', 'address', 'famsize', 'Pstatus', 
                             'schoolsup', 'famsup', 'internet', 'romantic']
        ...
        plt.tight_layout()
        plt.savefig('DataVisualizations/class_distribution_barcharts.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def grade_distribution_histogram(self):
        ...
        bins = np.arange(0, 21+1) - 0.5  # from -0.5 to 20.5 to center bins on integers
        
        # Create histogram
        counts, bins, patches = plt.hist(G3, bins=bins, alpha=0.7, color='skyblue', 
        ...
        for count, x in zip(counts, bin_centers):
            if count > 0:  # Only label non-empty bins
                plt.text(x, count + max_height * 0.02, str(int(count)),
                        ha='center', va='bottom', fontsize=9)
        ...
        plt.text(9.4, plt.ylim()[1]*0.95, 'Pass/Fail Threshold (10)', 
        ...
        plt.title('Distribution of Final Grades with Binary Classification Threshold', fontsize=15)
        plt.grid(alpha=0.3)
        ...
        # Save the figure
        plt.tight_layout()
        plt.savefig('DataVisualizations/grade_distribution_histogram.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def feature_correlation_matrix(self):
        ...
        for i, target in enumerate(['G1', 'G2', 'G3']):
            ...
            top_corrs = corr_matrix[target].drop(['G1', 'G2', 'G3']).abs().nlargest(5).index
        ...
        # Save the figure
        plt.tight_layout()
        plt.savefig('DataVisualizations/feature_correlation_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig('DataVisualizations/grades_correlation_matrix.png', dpi=300, bbox_inches='tight')
        plt.close()
